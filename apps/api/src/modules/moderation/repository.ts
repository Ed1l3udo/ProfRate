...
import { courses, disciplines, moderationLogs, professors, reviews, users } from "../../db/schema.js";
...
export function createModerationRepository(db: Database) {
  ...
  async function setReviewStatus(input: { id: number; status: "published" | "removed"; moderatorId?: number }) {
    ...
    return db.transaction(async (tx) => {
      const result = (await tx.update(reviews).set({ status: input.status, updatedAt: sql`now()` }).where(eq(reviews.id, input.id)).returning({ id: reviews.id, status: reviews.status })).at(0);
      if (result && input.moderatorId !== undefined) await tx.insert(moderationLogs).values({ moderatorId: input.moderatorId, action: input.status === "published" ? "review_approved" : "review_removed", targetType: "review", targetId: input.id });
      return result;
    });
  }
  ...
  async function setBlocked(input: { id: number; blocked: boolean; moderatorId?: number }) {
    return db.transaction(async (tx) => {
      const result = (await tx.update(users).set({ blocked: input.blocked, updatedAt: sql`now()` }).where(eq(users.id, input.id)).returning({ id: users.id, name: users.name, email: users.email, role: users.role, blocked: users.blocked })).at(0);
      if (result && input.moderatorId !== undefined) await tx.insert(moderationLogs).values({ moderatorId: input.moderatorId, action: input.blocked ? "user_blocked" : "user_unblocked", targetType: "user", targetId: input.id });
      return result;
    });
  }
  async function listLogs() { return db.select({ id: moderationLogs.id, action: moderationLogs.action, targetType: moderationLogs.targetType, targetId: moderationLogs.targetId, createdAt: moderationLogs.createdAt, moderator: { id: users.id, name: users.name } }).from(moderationLogs).leftJoin(users, eq(users.id, moderationLogs.moderatorId)).orderBy(sql`${moderationLogs.createdAt} desc`); }
  return { listReviews, setReviewStatus, listUsers, setBlocked, listLogs };
}
